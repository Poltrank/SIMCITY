...
export const MayorAuthModal: React.FC<MayorAuthModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'novo' | 'login' | 'perfil'>(initialTab);

  // New Game Form State
  const [titlePrefix, setTitlePrefix] = useState<'Prefeito' | 'Prefeita' | 'Dr.' | 'Dra.' | 'Nenhum'>('Prefeito');
  const [mayorNameInput, setMayorNameInput] = useState('Carlos Mendonça');
  ...
  const [selectedCity, setSelectedCity] = useState('Porto da Aliança');
  ...
  const handleCreateNewMayor = (e: React.FormEvent) => {
    ...
    const newProfile: MayorProfileRecord = {
      ...
      mayorName: finalMayorName,
      party: finalParty,
      partyAcronym: selectedPartyAcronym === 'CUSTOM' ? (customPartyAcronym.toUpperCase() || 'PBM') : selectedPartyAcronym,
      cityName: finalCity,
      governmentFocus: selectedFocus,
      ...
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-3xl shadow-2xl overflow-hidden flex flex-col my-auto max-h-[92vh]">
        ...
        <div className="flex items-center border-b border-slate-800 bg-slate-950/60 px-4 pt-2 gap-2">
          ...
          >
            ...
            Entrar / Login de Gabinete ({savedProfiles.length})
          </button>
        ...
        <div className="p-5 md:p-6 overflow-y-auto flex-1 space-y-6">
          ...
          {/* ABA 2: LOGIN / GABINETES SALVOS */}
          {activeTab === 'login' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-white">Selecione o Prefeito / Gabinete</h3>
                  <p className="text-xs text-slate-400">
                    Alterne instantaneamente entre diferentes mandatos e prefeituras gravadas neste navegador.
                  </p>
                </div>
                <button
                  onClick={() => setShowImportBox(!showImportBox)}
                  className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg text-xs font-bold border border-slate-700 transition-colors flex items-center gap-1.5"
                >
              ...
              {/* Lista de Gabinetes Salvos */}
              {savedProfiles.length === 0 ? (
                <div className="p-8 text-center text-slate-400 border border-dashed border-slate-800 rounded-xl space-y-3">
                  <Building2 className="w-10 h-10 mx-auto text-slate-600" />
                  <p className="text-sm font-semibold text-slate-300">Nenhum outro prefeito registrado</p>
                  <p className="text-xs text-slate-500 max-w-sm mx-auto">
                    Crie seu primeiro mandato na aba "Começar Novo Jogo" para salvar seus progressos!
                  </p>
                  <button
                    onClick={() => setActiveTab('novo')}
                    className="px-4 py-2 bg-amber-600 hover:bg-amber-500 text-slate-950 font-bold text-xs rounded-lg transition-colors"
                  >
                    Criar Novo Prefeito Agora
                  </button>
                </div>
              ) : (
                  ...
                  {savedProfiles.map((prof) => {
                    ...
                    return (
                      <div
                        ...
                        className={`cursor-pointer p-4 rounded-xl border transition-all flex flex-col md:flex-row md:items-center justify-between gap-3 ${
                          isCurrent
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <div className="w-10 h-10 rounded-lg bg-slate-800 border border-slate-700 flex items-center justify-center text-amber-300 font-black flex-shrink-0">
                            <Building2 className="w-5 h-5" />
                          </div>
                          <div>
                            <div className="flex items-center gap-2 mb-0.5">
                              <h4 className="font-bold text-white text-base">{prof.mayorName}</h4>
                              <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-blue-950 text-blue-300 border border-blue-600/40">
                            ...
                            <p className="text-xs text-slate-400 flex items-center gap-2">
                              <span className="text-slate-300 font-medium">{prof.cityName}</span>
                              <span>•</span>
                              <span>Mês {prof.termMonth} / {prof.year}</span>
                            </p>
                            <div className="flex items-center gap-4 mt-1.5 text-[11px] text-slate-400">
                              <span className="flex items-center gap-1 text-emerald-400 font-bold">
                                <DollarSign className="w-3.5 h-3.5" />
                                R$ {prof.treasury.toLocaleString()}
                              </span>
                              <span className="flex items-center gap-1">
                                <Users className="w-3.5 h-3.5 text-slate-500" />
                                {prof.population.toLocaleString()} hab.
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2 self-end md:self-center">
                          <button
                            onClick={() => handleLoadProfile(prof)}
                            className="px-3.5 py-1.5 bg-amber-600 hover:bg-amber-500 text-slate-950 font-bold text-xs rounded-lg transition-colors flex items-center gap-1"
                          >
                            Entrar no Gabinete
